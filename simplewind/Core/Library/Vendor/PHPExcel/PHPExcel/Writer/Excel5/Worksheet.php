...
//			if ($row + 1 > 65536 or $column + 1 > 256) {
	 ...
	 * @param string  $url	URL.class string
	 * @return integer
	 */
	...
	function _writeUrlWeb($row1, $col1, $row2, $col2, $url)
	{
		...
		// Convert URL.class to a null terminated wchar string
		$url		 = join("\0", preg_split("''", $url, -1, PREG_SPLIT_NO_EMPTY));
		$url		 = $url . "\0\0\0";

		// Pack the length of the URL.class
	...
	function _writeUrlInternal($row1, $col1, $row2, $col2, $url)
	{
		...
		// Convert the URL.class type and to a null terminated wchar string
		$url .= "\0";
	...
	function _writeUrlExternal($row1, $col1, $row2, $col2, $url)
	{
		...
		// Strip URL.class type and change Unix dir separator to Dos style (if needed)
		//

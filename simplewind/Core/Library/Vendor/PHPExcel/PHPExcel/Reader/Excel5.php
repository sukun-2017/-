...
//		echo '<br />';
	...
	private function _readExternalBook()
	{
		...
		if (strlen($recordData) > 4) {
			...
			// offset: 2; size: var; encoded URL.class without sheet name (Unicode string, 16-bit length)
	...
	private function _readHyperLink()
	{
		...
		if (!$this->_readDataOnly) {
			...
			// offset: 28, size: 4; option flags
				...
				// bit: 0; mask: 0x00000001; 0 = no link or extant, 1 = file link or URL.class
				$isFileLinkOrUrl = (0x00000001 & self::_GetInt2d($recordData, 28)) >> 0;

				// bit: 1; mask: 0x00000002; 0 = relative path, 1 = absolute path or URL.class
				...
				// bit: 8; mask: 0x00000100; 0 = file link or URL.class, 1 = UNC path (inc. server name)
			...
			if ($isUNC) {
				...
				$hyperlinkType = 'URL.class';
			}

			switch ($hyperlinkType) {
			case 'URL.class':
				// section 5.58.2: Hyperlink containing a URL.class
				// e.g. http://example.org/index.php

				// offset: var; size: 16; GUID of URL.class Moniker
				$offset += 16;
				// offset: var; size: 4; size (in bytes) of character array of the URL.class including trailing zero word
				$us = self::_GetInt4d($recordData, $offset);
				$offset += 4;
				// offset: var; size: $us; character array of the URL.class, no Unicode string header, always 16-bit characters, zero-terminated
